...
# Defining the oil element properties
class eDNA(Lagrangian3DArray):
    """Extending Lagrangian3DArray with specific properties for eDNA
    """
    ...
    variables = Lagrangian3DArray.add_variables([
        ('State', {'dtype': np.float32,
                          'units': 'n',
                          'default': 1}),
        ('degraded', {'dtype': np.int64,  # 0 for eggs, 1 for larvae
                     'units': '',
                     'default': 0})])
...
class eDNADrift(OceanDrift):
    ...
    required_variables = {
        ...
        'surface_downward_y_stress': {'fallback': 0},
        'turbulent_kinetic_energy': {'fallback': 0},
        'ocean_mixed_layer_thickness': {'fallback': 0},
        'turbulent_generic_length_scale': {'fallback': 0},
        'upward_sea_water_velocity': {'fallback': 0},
      }
    ...
    # The depth range (in m) which profiles shall cover
    required_profiles_z_range = [-1000, 0]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._add_config({
            'processes:degradation': {'type': 'bool', 'default': False,
                                      'description': 'eDNA degraded',
    ...
    def degradation(self):
        if self.get_config('processes:degradation') is True:
            '''
            eDNA degradation function based on the article:
            Tsuji et al. (2017), Water temperature-dependent degradation of environmental DNA 
            and its relation to bacterial abundance.
            '''
            # self.logger.debug('Calculating: degradation')

            #Random degradation of 0.001% each step
            #p = np.random.rand(self.num_elements_active(), )
            #D = p*self.elements.State
            #self.deactivate_elements(D > 0.999, reason = "degraded")

            # Random degradation of based on exponential model each step
            # p = np.random.exponential(scale = 1/1.5, size = self.num_elements_active())
            # D = p*self.elements.State
            # print(D)
            # self.deactivate_elements(D > 1.4, reason = "degraded")

            #Degrade 1% of particles each step
            #N = self.num_elements_active()
            #if N < 100:
            #    x = math.ceil(N * 0.01) #have to work in integers so this sorts out when x < 1
            #else:
            #    x = round(N * 0.01)
            #y = N - x
            #a = np.array(([1] * x) + ([0] * y))
            #np.random.shuffle(a)
            #D = a * self.elements.State
            #self.deactivate_elements(D > 0, reason="degraded")

            # Degrade 2 particles each step
            #N = self.num_elements_active()
            #x = int(N - 2) #number not to degrade
            #y = int(N) - x #number to degrade
            #a = np.array(([0] * x) + ([1] * y))
            #np.random.shuffle(a)
            #D = a * self.elements.State
            #self.deactivate_elements(D > 0, reason="degraded")

            swt_array = np.array(self.environment.sea_water_temperature)
            swt = np.mean(self.environment.sea_water_temperature) + 273.15 # Convert from Celsius to Kelvin
            # k = swt * 0.0001
            R = 8.314  # J/(mol K)
            E_a = 10000  # DNA degredation activation energy, using 1 kilajoule
            k = np.exp(-E_a / (R * swt)) #decay constant from temperature and Arrhenius equation
            seconds = self.time_step.total_seconds()
            age = seconds*self.steps_calculation/3600 #age in hours
            # k = 0.2972026 #degradation constant for Adriatic array
            # k = 0.0124122 #constant 
            N0 = self.num_elements_activated() #starting number
            N = self.num_elements_active()
            number_left = N0*(np.exp(-(k*age))) #exponential decay equation
            x = int(number_left)
            y = N - x
            a = np.array(([0] * x) + ([1] * y))
            np.random.shuffle(a)
    ...
    def update(self):
        ...
        #Deactivation
        #if self.get_config('deactivate_elements') is True:
            #N_active = self.num_elements_active()
            #self.deactivate_elements([0.5*N_active], reason  = "decayed") #("condition" = True, reason = 'some reason')

        #Max age
        if self.get_config('drift:max_age_seconds') is True:
