...
class eDNADrift(OceanDrift):
    ...
    def degradation(self):
        if self.get_config('processes:degradation') is True:
            ...
            ###swt = self.environment.sea_water_temperature
            seconds = self.time_step.total_seconds()
            age = seconds*self.steps_calculation/3600 #age in hours
            # Decay rate in hours (temperature in Celsius)
            ###tau = -(0.01*swt-0.07)
            #k = 0.2972026 #degradation constant for Chagos array
            k = 0.1124122 #constant for sicilian channel in June
