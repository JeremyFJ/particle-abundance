...
setwd("/Volumes/Nick Dunn SP/PhD/Med_data/Results/Grid")
setwd("./19062021_K0.11")
...
for(i in 1: 122){
  ...
  Start_longitude_main <- substr(file,start=6, stop=7)
  Start_longitude_decimal <- substr(file,start=9, stop=9)
  Start_longitude <- paste(Start_longitude_main,".",Start_longitude_decimal, sep = "")
  
  ### pulling out the longitude from the file name 
  Start_latitude_main <- substr(file,start=11, stop=12)
  Start_latitude_decimal <- substr(file,start=14, stop=14)
  Start_latitude <- paste(Start_latitude_main,".",Start_latitude_decimal, sep = "")
  ...
  all_locations$date <- 19062021

  write.csv(all_locations, 
            file = paste("~/opendrift/Med_data/Results/Grid/CSV/190621/19062021_K0.11", EXP_files[i], ".csv", sep = "_"),
            row.names = FALSE)
...
df <- list.files(path="~/opendrift/Med_data/Results/Grid/CSV/190621", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  bind_rows
df <- as.data.frame(df)
head(df)
write.csv(df, "~/opendrift/Med_data/Results/Grid/CSV/190621/all_csvs_190621.csv")

#Read in all combined csvs
df <- list.files(path="/Volumes/Nick Dunn SP/PhD/Med_data/Results/CSVs/Combined_csvs", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  bind_rows
